...
buffer_config = BufferConfig(obs_shape=obs_shape, setting="MC")
...
def test_A2C_basics():
    agent = A2C(A2C_CONFIG, LoggingConfig(logging_output=None))
    obs, info = ENV.reset()
...
def test_A2C_train_test():
    # MC
    MC_agent = A2C(A2C_CONFIG, LoggingConfig(tensorboard=False, wandb=False))
    n_episodes = 3
    ...
    n_step_agent = A2C(n_step_A2C_CONFIG, LoggingConfig(tensorboard=False, wandb=False))
    n_episodes = 100
