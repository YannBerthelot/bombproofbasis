...
def test_add_buffer():
    ...
    with pytest.raises(ValueError):
        faulty_buffer_config = BufferConfig(
            setting="MC", gamma=1.99, buffer_size=10, n_steps=5
        )
        RolloutBuffer(faulty_buffer_config)
    with pytest.raises(ValueError):
        faulty_buffer_config = BufferConfig(
            setting="MC", gamma=0.99, buffer_size=-1, n_steps=5
        )
        RolloutBuffer(faulty_buffer_config)
    with pytest.raises(ValueError):
        faulty_buffer_config = BufferConfig(
            setting="MC", gamma=0.99, buffer_size=10, n_steps=-1
        )
        RolloutBuffer(faulty_buffer_config)
    buffer_size = 10
    buffer_config = BufferConfig(
        setting="MC", gamma=0.99, buffer_size=buffer_size, n_steps=2
    )
...
def test_update_advantages():
    ...
    values = torch.Tensor(np.ones(buffer_size))
